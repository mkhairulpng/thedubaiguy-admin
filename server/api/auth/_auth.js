...
function isAdmin(req){const password=process.env.ADMIN_PASSWORD, token=cookie(req);if(!password||!token)return false;try{const [p,s]=token.split('.');if(!p||!s||sign(p,String(password))!==s)return false;const d=JSON.parse(Buffer.from(p,'base64url').toString());return d.role==='admin'&&d.exp>Date.now()}catch{return false}}
