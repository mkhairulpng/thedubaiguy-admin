...
/* Replace the existing Aura mark with the supplied Aura wordmark, rendered in white. */
.tdg-aura-wordmark{
  ...
  function replaceAuraWordmarks(){
    document.querySelectorAll('.tdg-aura-logo-compact, .tdg-aura-side-logo-compact').forEach(function(el){
      if(el.tagName==='IMG'){
        ...
        el.classList.remove('tdg-aura-logo-compact','tdg-aura-side-logo-compact');
        el.classList.add(el.matches('.tdg-aura-side-logo-compact')?'tdg-aura-side-wordmark':'tdg-aura-wordmark');
      }else{
        const img=document.createElement('img');
        img.src=src;
        img.alt='AURA';
        img.className=el.classList.contains('tdg-aura-side-logo-compact')?'tdg-aura-side-wordmark':'tdg-aura-wordmark';
        el.replaceWith(img);
      }
    });
  }
