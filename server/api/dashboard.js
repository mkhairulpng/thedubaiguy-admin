...
.tdg-ga-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px;margin-bottom:8px}
...
.tdg-ga-table{width:100%;border-collapse:collapse;font-size:9px;min-width:720px}
...
@media(max-width:900px){.tdg-ga-summary{grid-template-columns:repeat(2,1fr)}.tdg-ga-table{min-width:680px}}
</style>
<script>
(function(){
  ...
  function render(d){
    ...
    const cities=(d.activeUsersByCity||[]).slice(0,8);
...
</script>`;  return html.replace('</body>',kpiCompactPatch+patch+auraPatch+compactPatch+auraContentPatch+topLeftLogoPatch+auraKpiExactPatch+auraKpiMirrorPatch+netsSettlementPatch+auraWordmarkPatch+posCatalogPatch+observabilityPatch+'</body>');
}
