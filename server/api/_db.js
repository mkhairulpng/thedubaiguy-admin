...
function getPool() {
  ...
  if (!url) {
    const e = new Error('Database is not configured. Add DATABASE_URL (or POSTGRES_URL) in Vercel Environment Variables.');
    e.statusCode = 503;
    throw e;
  }
  pool = new Pool({ connectionString: url, max: 5, idleTimeoutMillis: 10000, connectionTimeoutMillis: 5000, ssl: { rejectUnauthorized: false } });
  return pool;
}
  ...
  schemaPromise = (async () => {
    ...
    await db.query(`
      ...
      CREATE TABLE IF NOT EXISTS customers (
        ...
        aura_member BOOLEAN NOT NULL DEFAULT FALSE,
        aura_balance NUMERIC(12,2) NOT NULL DEFAULT 0,
        aura_cashback_earned NUMERIC(12,2) NOT NULL DEFAULT 0,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      );
