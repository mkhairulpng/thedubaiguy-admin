...
## Hobby deployment architecture

v254-Hobby consolidates the API into a single Vercel catch-all function at `api/[...route].js`. The implementation modules live under `server/api/`, so Vercel does not count each module as a separate Serverless Function. Public API URLs remain unchanged (`/api/products`, `/api/orders`, `/api/auth/login`, `/api/stripe/*`, `/api/lalamove/*`, etc.).
